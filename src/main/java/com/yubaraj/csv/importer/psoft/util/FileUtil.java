...
private static final Logger LOGGER = LoggerFactory.getLogger(FileUtil.class);
	...
	try {
	    ...
	    LOGGER.error("File path not found. Path:"+location);
	    e.printStackTrace();
	    return null;
	}
    }
	...
	// getFiles("C:\\psoft-files\\csv-files"));
	// System.out.println("size: " +
	// getFiles("C:\\psoft-files\\csv-files").size());
	...
	Files.move(FileSystems.getDefault().getPath(source), FileSystems.getDefault().getPath(target),
	...
	if (files != null && files.length > 0) {
		...
		if (file.isFile()) {
			...
			Files.move(FileSystems.getDefault().getPath(source, file.getName()),
				FileSystems.getDefault().getPath(target), StandardCopyOption.ATOMIC_MOVE);
		    } catch (IOException e) {
		    }
		}
	    }
